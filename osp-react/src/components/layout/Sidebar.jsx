...
export default function Sidebar({ isOpen, onClose, collapsed = false }) {
  ...
  // Menu yg punya child aktif otomatis kebuka — dihitung langsung dari
  // location.pathname (derived), bukan disinkronkan lewat effect+state.
  const activeParentLabel =
    SIDEBAR_NAV.find((item) => item.children?.some((c) => c.path === location.pathname))
      ?.label ?? null;

  const isMenuOpen = (label) => openMenus[label] ?? label === activeParentLabel;
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 h-full z-50 transform transition-all duration-300 ease-in-out flex flex-col flex-shrink-0 lg:translate-x-0 lg:static lg:z-auto ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        } ${collapsed ? 'w-20' : 'w-64'}`}
        style={{ backgroundColor: 'var(--sidebar-bg)', borderRight: '1px solid var(--sidebar-border)' }}
      >
        {/* Logo */}
        <div className="h-16 flex items-center justify-center gap-3 px-3 flex-shrink-0"
          style={theme.invertLogo === false ? undefined : { filter: 'brightness(0) invert(1)' }}
        >
        <img
        src={theme.logoUrl}
        alt={theme.brandName}
        className={`w-auto object-contain ${collapsed ? 'h-9 max-w-[36px]' : 'h-25 max-w-[160px]'}`}
        style={theme.invertLogo === false ? undefined : { filter: 'brightness(0) invert(1)' }}
      />
</div>

        {/* User info — disembunyikan saat collapsed */}
        {!collapsed && (
          <div
        ...
        {/* Navigation */}
        <nav className={`flex-1 overflow-y-auto py-3 space-y-1 sidebar-scrollbar ${collapsed ? 'px-2' : 'px-3'}`}>
          {SIDEBAR_NAV.map((item) => {
            ...
            if (!item.children) {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center rounded-lg text-sm font-medium transition-colors sidebar-nav-item ${
            ...
            // Menu dengan children (collapsible) — submenu disembunyikan saat collapsed
            const menuOpen = !collapsed && isMenuOpen(item.label);
            return (
              <div key={item.label}>
                <button
                  onClick={() => toggleMenu(item.label)}
                  title={collapsed ? item.label : undefined}
                  className={`flex items-center w-full rounded-lg text-sm font-medium transition-colors sidebar-nav-item ${
